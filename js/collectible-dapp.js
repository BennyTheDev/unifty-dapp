function TncDapp() {
  ...
  const _this = this;
  this.collectibleTemplate = Handlebars.compile(
    $("#collectible-template").html()
  );
  ...
  this.render = async function (erc1155, id, index, category) {
    ...
    if (index) {
      ...
      options =
        ask.seller.toLowerCase() == tncLib.account.toLowerCase() ? "true" : "";
      multiplier = ask.erc1155Address.length - 1;
      seller = ask.seller;
      royalties = _this.formatNumberString(
        await tncLibMarket.getRoyalties(ask.erc1155Address[0], ask.id[0]),
        2
      );
    }
    ...
    let tmpl = _this.collectibleTemplate({
      ...
      collectionName:
        meta.name != "n/a"
          ? '<div class="text-truncate">' + meta.name + "</div>"
          : '<div class="text-truncate">' + erc1155 + "</div>",
    ...
    if (allowance.lt(fullPrice)) {
      ...
      tncLibMarket.buy(
        ...
        function () {
          ...
          $(_button).html("Acquire!");
          toastr["error"](
            "An error occurred with your buying transaction.",
            "Error"
          );
        }
      );
  ...
  this.loadPage = async function (page) {
    ...
    switch (page) {
      default:
        ...
        if (
          !(await tncLib.isErc1155Supported(_this.getUrlParam("collection")))
        ) {
        ...
        _this.render(
          _this.getUrlParam("collection"),
          _this.getUrlParam("id"),
          _this.getUrlParam("market_index"),
          _this.getUrlParam("market_category")
        );
