function TncDapp() {
  ...
  this.displayBalance = function () {
    ...
    $(document).ready(async function () {
      setInterval(async function () {
        let balance = await tncLib.balanceOfErc20Raw(
          "0xa7814452481532987BEFa93cE55EE7CC60aAb916",
          tncLib.account
        );
        ...
        balance = parseInt(
          _this.formatNumberString(balance, 18)
        ).toLocaleString();
  ...
  this.render = async function (
    ...
    if (
      chain_id == "1" &&
      erc1155.toLowerCase() == tncLibBridgeIn.uniftyverseAddress.toLowerCase()
    ) {
    ...
    if (which == "picker" || which == "request" || which == "request2") {
      ...
      let tmpl = _this.pickerTemplate({
        ...
        options:
          sellerAddress.toLowerCase() == tncLib.account.toLowerCase()
            ? "true"
            : "",
        collectionName:
          meta.name != "n/a"
        ...
        opensea:
          ...
          "&market_index=" +
          (category > 0
            ? category_index + "&market_category=" + category
            : index + "&market_category=0"),
      });
  ...
  this.performCancellation = async function () {
    ...
    tncLibMarket.cancel(
      ...
      function () {
        ...
        $(_button).html("Cancel");
        toastr["error"](
          "An error occurred with your cancellation transaction.",
          "Error"
        );
      }
    );
  ...
  this.performBatchBuy = async function (index) {
    ...
    if (ask.seller.toLowerCase() == tncLib.account.toLowerCase()) {
      _alert("You cannot aquire your own sale.");
      return;
    }

    let balance = web3.utils.toBN(
      await tncLib.balanceOfErc20Raw(ask.tokenAddress, tncLib.account)
    );
    ...
    if (allowance.lt(fullPrice)) {
      _alert("Please approve first, then click the aquire button again.");
      ...
      await window.tncLib.approveErc20(
        ...
        function (receipt) {
          ...
          toastr["success"]("Transaction has been finished.", "Success");
          $(_button).prop("disabled", false);
          $(_button).html("Acquire Now!");
          $("#nftBatchBuy" + index).prop("disabled", false);
          $("#nftBatchBuy" + index).html("Acquire");
      ...
      tncLibMarket.buy(
        ...
        localStorage.getItem("marketRef") != null
          ? localStorage.getItem("marketRef")
        ...
        function () {
          ...
          $(_button).html("Acquire!");
          toastr["error"](
            "An error occurred with your acquiring transaction.",
            "Error"
          );
        }
      );
  ...
  this.performBuy = async function () {
    ...
    for (let i = 0; i < ask.erc1155Address.length; i++) {
      if (
        ask.erc1155Address[i].toLowerCase() ==
          $("#nftBuyErc1155Address").val().toLowerCase() &&
        ...
        if (ask.seller.toLowerCase() == tncLib.account.toLowerCase()) {
          _alert("You cannot acquire your own sale.");
          return;
        }
        ...
        if (allowance.lt(fullPrice)) {
          ...
          await window.tncLib.approveErc20(
            ...
            function (receipt) {
              ...
              $("#nftBuyButtonShortcut" + index).html("Acquire");
              $("#alertModal").modal("hide");
            },
            function () {
              toastr.remove();
          ...
          tncLibMarket.buy(
            ask.seller,
            amount.toString(),
            _this.getUrlParam("ref") != null
              ? _this.getUrlParam("ref")
              : "0x0000000000000000000000000000000000000000",
            index,
            function () {
              toastr["info"](
  ...
  this.performSwapRequest = async function () {
    ...
    let nif = web3.utils.toBN(
      _this.resolveNumberString($("#nftSwapNif").val().trim(), 18)
    );
    ...
    if (ask.updates != 0) {
      _alert(
        "The opposing offer has changed since it has been released. Swapping is not possible when the conditions changed."
      );
      return;
    }
    ...
    if (ask1.updates != 0) {
      _alert(
        "Your offer has changed since you have been releasing it. Swapping is not possible when the conditions changed."
      );
      return;
    }
    ...
    let balance = web3.utils.toBN(
      await tncLib.balanceOfErc20Raw(tncLib.nif.options.address, tncLib.account)
    );
    ...
    if (allowance.lt(nif)) {
      ...
      tncLibMarket.requestSwap(
        ...
        async function (receipt) {
          ...
          _alert(
            "Swap request successful. If your request is getting accepted, the swap will be performed. You can cancel your request at any time."
          );
        },
        function () {
          toastr.remove();
          ...
          $(_button).html("Swap!");
          toastr["error"](
            "An error occurred with your swapping transaction.",
            "Error"
          );
        }
      );
  ...
  this.populateTokenLookup = async function (e) {
    ...
    switch (chain_id) {
      case "64": // xDai
        ...
        var o2 = new Option(
          "NIF (Unifty)",
          "0x1A186E7268F3Ed5AdFEa6B9e0655f70059941E11"
        );
        $(o2).html("NIF (Unifty)");
      ...
      case "4d": // xDai (SPOA) Testnet
        var o = new Option(
          "NIF (Unifty)",
          "0x93fEB07f2823600DD3b9EFFd9356de10C387d9d7"
        );
        $(o).html("NIF (Unifty)");
        $("#lookupToken").append(o);
      ...
      case "38": // BSC MAINNET
        var o2 = new Option(
          "bNIF (Unifty)",
          "0x3aD4eC50f30dAb25C60e0e71755AF6B9690B1297"
        );
        $(o2).html("bNIF (Unifty)");
        $("#lookupToken").append(o2);
      ...
      case "61": // BSC TESTNET
        var o = new Option(
          "NIF (Unifty)",
          "0xaC636E43b2a3e8654c993c4c5A72a2cDc41Db0FF"
        );
        $(o).html("NIF (Unifty)");
        $("#lookupToken").append(o);
        break;
    ...
    $("#withdrawFundsButton").on("click", async function () {
      ...
      tncLibMarket.withdrawFunds(
        ...
        function () {
          ...
          $(_button).html("Withdraw");
          toastr["error"](
            "An error occurred with your withdrawal transaction.",
            "Error"
          );
        }
      );
  ...
  this.getWalletNfts = async function (address) {
    ...
    for (let i = length - 1; i >= 0; i--) {
      ...
      for (let j = 0; j < nfts.length; j++) {
        if (
          (await tncLib.balanceof(myCollection.erc1155, address, nfts[j])) > 0
        ) {
  ...
  this.renderSellSelection = async function (erc1155Address, id, address) {
      ...
      if (typeof data == "object") {
        ...
        data_interactive_url =
          typeof data.interactive_url != "undefined" && data.interactive_url
            ...
            : "";
        data_name =
          typeof data.name != "undefined" && data.name ? data.name : "#" + id;
        data_description =
          typeof data.description != "undefined" && data.description
            ? data.description
            : "";
    ...
    if (typeof data_image != "undefined" && data_image != "") {
      image =
        '<img src="' + data_image + '" border="0" style="width: 50px;" />';
    }
    ...
    out += '<div class="col-3">';
    out +=
      '<input type="text" value="" placeholder="0" style="width: 50px;" data-erc1155address="' +
      erc1155Address +
      '" data-id="' +
      id +
      '" id="amountToAdd' +
    ...
    $("#amountToAdd" + erc1155Address + id).on("change", async function (e) {
      ...
      if (!isNaN(amount) && amount > 0) {
        let balance = await tncLib.balanceof(
        ...
        if (balance < amount) {
          _alert(
            "Insufficient balance. You own " + balance + " items of this NFT."
          );
          return;
        }
    ...
    $("#nftSellCustomTokenAddress2").on("change", async function () {
      ...
      if (await web3.utils.isAddress(token)) {
        try {
          ...
          $("#nftSellCustomTokenAddressInfo2").text(
  ...
  this.swapOtherRequests = async function () {
    ...
    for (let i = length - 1; i >= 0; i--) {
      ...
      let listEntry = await tncLibMarket.getSwapRequestListEntry(
        tncLib.account,
        i
      );

      let request = await tncLibMarket.getSwapRequest(
        listEntry.seller,
        listEntry.swapIndex
      );
      ...
      if (nifAmount.gt(zero)) {
        ...
        extraNif =
          '<div style="margin-top: -10px; color: #ffffff; font-size: 2rem;" class="mb-1">+' +
          extraNif.substring(0, extraNif.length - 10) +
          " NIF</div>";
      }
      ...
      out +=
        '</div><div class="col" style="text-align: center; margin-bottom: 50px;">';

      let info = "";

      if (!request.settled && !request.rejected) {
        out +=
          '<button type="button" class="cancelSwapButton btn btn-danger mt-lg-5" data-seller="' +
          ask0.seller +
          '" data-swapindex="' +
          listEntry.swapIndex +
          '" id="cancelSwapButton' +
  ...
  this.swapRequests = async function () {
    ...
    for (let i = length - 1; i >= 0; i--) {
      ...
      if (
        ...
        out +=
          '<div class="col" style="text-align: center; margin-bottom: 50px;"><button type="button" class="acceptSwapButton btn btn-primary mt-lg-5" data-swapindex="' +
          i +
          '" id="acceptSwapButton' +
          i +
          '">Accept</button></div>';
      ...
      if (info != "") {
        out +=
          '<div class="col" style="text-align: center; margin-bottom: 50px; vertical-align: middle; margin-top: -50px;"><div style="margin-top: 50px;">' +
          info +
          "</div></div>";
      }
  ...
  this.performSwapCancel = async function (e) {
    ...
    tncLibMarket.cancelSwapRequest(
      ...
      function () {
        ...
        $(_button).html("Cancel");
        toastr["error"](
          "An error occurred with your cancelling transaction.",
          "Error"
        );
      }
    );
  ...
  this.performSell = async function () {
    ...
    window.tncLibMarket.sell(
      ...
      function () {
        ...
        $("#nftSellButton").html("Sell!");
        toastr["error"](
          "An error occurred with your sell transaction.",
          "Error"
        );
      }
    );
