function TncDapp() {
      ...
      try {
        ...
        fetchUrl(
          api_url +
            "1.0/" +
            chain_id +
            "/collections/events/URI/erc1155Address/" +
            pool.erc1155 +
            "/id/0",
          5000
        );
  ...
  this.populateMyNfts = async function () {
    ...
    if (offset <= 0) {
      $("#myNftsPage").append(
        '<div class="container-fluid" style="grid-column-start: 1; grid-column-end: -1;"><h1 id="nftContractNameTitle"></h1></div>'
      );
    ...
    $("#nftBurnButton").on("click", function () {
      ...
      window.tncLib.burn(
        ...
        function () {
          toastr.remove();
          toastr["error"](
            "An error occurred with your burn transaction.",
            "Error"
          );
        }
      );
    ...
    $("#nftMintButton").on("click", function () {
      ...
      window.tncLib.mint(
        ...
        function () {
          toastr.remove();
          toastr["error"](
            "An error occurred with your mint transaction.",
            "Error"
          );
        }
      );
    ...
    $("#nftTransferButton").on("click", function () {
      ...
      window.tncLib.transfer(
        ...
        function () {
          toastr.remove();
          toastr["error"](
            "An error occurred with your transfer transaction.",
            "Error"
          );
        }
      );
  ...
  this.newUpdateCollection = async function (e) {
    ...
    if ($(e.relatedTarget).data("edit")) {
      ...
      let contractMeta = await window.tncLib.getErc1155Meta(
        $(e.relatedTarget).data("contractAddress")
      );
      ...
      try {
        ...
        if (typeof data == "object") {
          ...
          if (data.animation_url.trim() != "") {
            let tag =
              '<video autoplay="autoplay" preload="auto" muted="muted" poster=' +
              JSON.stringify(data.image) +
              ' loop="loop" width="400" controls="controls">\n' +
              "  <source src=" +
              JSON.stringify(data.animation_url) +
              ' type="video/mp4">\n' +
          ...
          if (data.interactive_url.trim() != "") {
            let tag =
              ...
              JSON.stringify(data.interactive_url.trim()) +
  ...
  this.processTransferMultiBatch = async function () {
    let recipients = $("#nftTransferMultiBatchToAddresses")
      .val()
      .trim()
      .replace("\r")
      .split("\n");
    let collection = $("#nftTransferMultiBatchCollection").val().trim();
    let ids = $("#nftTransferMultiBatchIds")
      .val()
      .trim()
      .replace("\r")
      .split("\n");
    let amounts = $("#nftTransferMultiBatchAmounts")
      .val()
      .trim()
      .replace("\r")
      .split("\n");
  ...
  this.performRoyalties = async function () {
    ...
    window.tncLibMarket.setRoyalties(
      ...
      function () {
        ...
        $("#storeRoyaltiesButton").html("Set Royalties");
        toastr["error"](
          "An error occurred with your royalties transaction.",
          "Error"
        );
      }
    );
  ...
  this.populateRoyalties = async function (e) {
    ...
    $("#nftRoyalties").val(
      _this.formatNumberString(
        await tncLibMarket.getRoyalties(
          $(e.relatedTarget).data("contractAddress"),
          $(e.relatedTarget).data("nftId")
        ),
        2
  ...
  this.loadPage = async function (page) {
    ...
    switch (page) {
      ...
      default:
        ...
        $("#nftTransferMultiBatchButton").on(
          "click",
          this.processTransferMultiBatch
        );
        _this.clearNewUpdateNft();
        $("#myPoolsPage").css("display", "grid");
        $("#view-collections").css("display", "none");
  ...
  this.storeIpfsImage = function (fileElement, urlStorageElement) {
    ...
    reader.onloadend = function () {
      ...
      ipfs.add(buf, (err, result) => {
        ...
        $(".imageFileDisplay").html(
          "<img src=" + JSON.stringify(ipfsLink) + ' border="0" width="200"/>'
        );
        $(".submitNewUpdate").prop("disabled", false);
  ...
  this.storeIpfsVideo = function (fileElement, urlStorageElement) {
    ...
    reader.onloadend = function () {
      ...
      ipfs.add(buf, (err, result) => {
        ...
        let tag =
          '<video autoplay="autoplay" preload="auto" muted="muted" loop="loop" width="400" controls="controls">\n' +
          "  <source src=" +
          JSON.stringify(ipfsLink) +
          ' type="video/mp4">\n' +
          "  Videos are not supported by your browser\n" +
          "</video>";
        $(".videoFileDisplay").html(tag);
  ...
  $("#addAttributeButton").on("click", async function () {
    ...
    for (let val in data) {
      ...
      $(opt).prop(
        "selected",
        val == $("#addDisplayType").val().trim() ? true : false
      );
      $(opt).appendTo(s);
    }
    ...
    newThing.setAttribute(
      "class",
      "addedAttributeDelete btn btn-secondary ml-3 mt-3"
    );
  ...
  document.getElementById("nftImageFile").onchange = function () {
    _this.storeIpfsImage(
      document.getElementById("nftImageFile"),
      document.getElementById("nftImageUrl")
    );
  };

  document.getElementById("nftAudioFile").onchange = function () {
    _this.storeIpfsAudio(
      document.getElementById("nftAudioFile"),
      document.getElementById("nftAudioUrl")
    );
  };

  document.getElementById("nftVideoFile").onchange = function () {
    _this.storeIpfsVideo(
      document.getElementById("nftVideoFile"),
      document.getElementById("nftVideoUrl")
    );
  };

  document.getElementById("nftInteractiveFile").onchange = function () {
    _this.storeIpfsInteractive(
      document.getElementById("nftInteractiveFile"),
      document.getElementById("nftInteractiveUrl")
    );
  };
  ...
  document.getElementById("erc1155ImageFile").onchange = function () {
    _this.storeIpfsImage(
      document.getElementById("erc1155ImageFile"),
      document.getElementById("erc1155ImageUrl")
    );
  };
  ...
  $("#nftSubmit").on("click", async function () {
    if (
      ($("#nftIsEdit").val() == "0" &&
        parseInt($("#nftSupply").val().trim()) < 0) ||
      ($("#nftIsEdit").val() == "0" &&
        parseInt($("#nftMaxSupply").val().trim()) <= 0) ||
      $("#nftName").val().trim() == ""
    ) {
    ...
    if (
      $("#nftIsEdit").val() == "0" &&
      (isNaN(parseInt($("#nftSupply").val().trim())) ||
    ...
    if (
      $("#nftIsEdit").val() == "0" &&
      (parseInt($("#nftSupply").val().trim()) < 0 ||
        parseInt($("#nftMaxSupply").val().trim()) <
          parseInt($("#nftSupply").val().trim()))
    ) {
      _alert(
        "Supply and Max. Supply must be larger or equal to zero and Max. Supply must equal or larger than Supply."
      );
      return;
    }
    ...
    for (let i = 0; i < keys.length; i++) {
      if ($(types[i]).val().trim() != "text") {
        attributes.push({
          trait_type: $(keys[i]).val().trim(),
          value: $(values[i]).val().trim(),
          display_type: $(types[i]).val().trim(),
        });
      } else {
        attributes.push({
          trait_type: $(keys[i]).val().trim(),
      ...
      if ($("#nftIsEdit").val() == "0") {
        ...
        await window.tncLib.newNft(
          ...
          function () {
            toastr.remove();
            toastr["error"](
              "An error occurred with your new NFT transaction.",
              "Error"
            );
          }
        );
        ...
        await window.tncLib.updateUri(
          ...
          function () {
            toastr.remove();
            toastr["error"](
              "An error occurred with your NFT Update transaction.",
              "Error"
            );
          }
        );
  ...
  $("#erc1155Submit").on("click", async function () {
    ...
    ipfs.add(
      buffer.Buffer(JSON.stringify(contractInfo)),
      async (err, result) => {
        console.log(err, result);
          ...
          await window.tncLib.newErc1155(
            ...
            function () {
              try {
                toastr.remove();
                toastr["error"](
                  "An error occurred with your new Collection transaction. Do you have sufficient funds?",
                  "Error"
                );
              } catch (e) {
                console.log(e);
          ...
          await window.tncLib.setContractURI(
            ...
            function () {
              try {
                toastr.remove();
                toastr["error"](
                  "An error occurred with your updated Collection transaction.",
                  "Error"
                );
              } catch (e) {
                console.log(e);
