function TncDapp() {
  ...
  const _this = this;
  this.farmHeaderTemplate = Handlebars.compile(
    $("#farm-header-template").html()
  );
  ...
  this.populateFarm = async function (farmAddress) {
    let isAdmin = await tncLib.farmIsWhitelistAdmin(
      tncLib.account,
      farmAddress
    );
    ...
    if (runMode == 2) {
      ...
      farmBalance = web3.utils.toBN(
        await tncLib.farmBalanceOfRaw(farmAddress, tncLib.account)
      );
      if (farmBalance.eq(_null)) {
        ...
        if (
          !isAdmin &&
          0 ==
            (await tncLib.farmPendingWithdrawals(tncLib.account, farmAddress))
        ) {
    ...
    $("#nftStakeButton").on("click", async function () {
      ...
      if (added == 0 || added < minStakes || added > maxStakes) {
        ...
        let maxStakesLeft = web3.utils
          .toBN(await tncLib.farmMaxStakeRaw(farmAddress))
          .sub(web3.utils.toBN(balance));
        _alert(
          "Please stake with the proper amounts.<br />Left for staking: " +
            _this.formatNumberString(maxStakesLeft.toString(), decimals) +
            " " +
            symbol +
            "<br /> " +
            (minStakes == 0
              ? ""
              : "Minimum Stake: " +
                _this.formatNumberString(minStakes, decimals) +
                " " +
                symbol +
                "<br />") +
            "Maximum Stake: " +
            _this.formatNumberString(maxStakes, decimals) +
            " " +
            symbol
        );
        return;
        ...
        if (typeof data == "object") {
          ...
          farm_image =
            typeof data.image != "undefined" && data.image
              ...
              : "";
          farm_twitter =
            typeof data.twitter != "undefined" && data.twitter
              ? data.twitter
              : "";
          farm_discord =
            typeof data.discord != "undefined" && data.discord
              ? data.discord
              : "";
          farm_instagram =
            typeof data.instagram != "undefined" && data.instagram
              ? data.instagram
              : "";
          farm_youtube =
            typeof data.youtube != "undefined" && data.youtube
              ? data.youtube
              : "";
          farm_web = typeof data.web != "undefined" && data.web ? data.web : "";
          farm_email =
            typeof data.email != "undefined" && data.email ? data.email : "";
          farm_phone =
            typeof data.phone != "undefined" && data.phone ? data.phone : "";
          farm_telegram =
            typeof data.telegram != "undefined" && data.telegram
              ? data.telegram
              : "";
          farm_medium =
            typeof data.medium != "undefined" && data.medium ? data.medium : "";
    ...
    for (let i = offset; i < nfts.length; i++) {
      ...
      if (nft.uri.includes("api.opensea.io")) {
        ...
        if (nftUri.length > 0 && nftUri[nftUri.length - 1].startsWith("0x")) {
          nftUri[nftUri.length - 1] = nftUri[nftUri.length - 1].replace(
            "0x",
            ""
          );
          nft.uri = nftUri.join("/");
      ...
      let farm_data = await window.tncLib.farmNftData(
        farmAddress,
        nfts[i].erc1155,
        nfts[i].id
      );
      ...
      let stock = await tncLib.balanceof(
        nfts[i].erc1155,
        farmAddress,
        nfts[i].id
      );
      ...
      if (hasShop && runMode != 0) {
        let prices = await tncLib.farmShopGetPrice(
          addonAddress,
          nfts[i].erc1155,
          nfts[i].id
        );
      ...
      let tmpl = _this.farmTemplate({
        ...
        opensea:
          chain_id == "1" || chain_id == "4"
            ? "https://opensea.io/assets/" + nfts[i].erc1155 + "/" + nfts[i].id
            : "collectible.html?collection=" +
              nfts[i].erc1155 +
              "&id=" +
              nfts[i].id,
      });
      ...
      if (hasShop && runMode != 0) {
        $("#farmSetShopPrice" + nfts[i].erc1155 + nfts[i].id).css(
          "display",
          "block"
        );
      } else {
      ...
      if (runMode == 2) {
        if (farmBalance.eq(_null)) {
          $("#redeemButton" + nfts[i].erc1155 + nfts[i].id).css(
            "display",
            "none"
          );
        }
      ...
      $("#buyButton" + nfts[i].erc1155 + nfts[i].id).on(
        "click",
        async function () {
          ...
          let _price = web3.utils
            .toBN(prices[0])
            .add(web3.utils.toBN(prices[1]));

          $("#farmBuyPrice").html(
            _this.formatNumberString(_price.toString(), 18)
          );
          ...
          $("#buyButton").on("click", async function () {
            ...
            let farmBalance = await tncLib.balanceof(
              $("#farmBuyAddress").val(),
              farmAddress,
              $("#farmBuyId").val()
            );
            ...
            let balance = web3.utils.toBN(
              await web3.eth.getBalance(tncLib.account)
            );
            if (balance.lt(final)) {
      ...
      $("#redeemButton" + nfts[i].erc1155 + nfts[i].id).on(
        "click",
        async function () {
          let user_balance = await tncLib.farmPointsEarned(
            farmAddress,
            tncLib.account
          );
          ...
          if (user_balance >= $(this).data("points") && stock > 0) {
            ...
            let nifBalance = web3.utils.toBN(
              await tncLib.nif.methods
                .balanceOf(tncLib.account)
                .call({ from: tncLib.account })
            );
            ...
            await tncLib.farmRedeem(
              ...
              erc1155,
              nfts[i].id,
              !(await tncLib.farmIsCloned(farmAddress)) &&
                (haveWildcard || nifBalance.gte(feeMinNif))
                ? "0"
                : nfts[i].mintFee.toString(),
              function () {
                toastr["info"](
                  "Please wait for the transaction to finish.",
                  "Redeem...."
              ...
              function (err) {
                toastr.remove();
                toastr["error"](
                  "An error occurred with your redeem transaction.",
                  "Error"
                );
              }
            );
  ...
  this.updateRegisteredCollections = async function (address) {
    ...
    if (localStorage.getItem("collectionAddresses" + chain_id)) {
      collectionAddresses = JSON.parse(
        localStorage.getItem("collectionAddresses" + chain_id)
      );
    }
    ...
    if (!collectionAddresses.includes(address)) {
      ...
      localStorage.setItem(
        "collectionAddresses" + chain_id,
        JSON.stringify(collectionAddresses)
      );
    }
  };
    ...
    if (farm_address != null && (await tncLib.isFarm(farm_address))) {
      $(".redeemButton").each(async function () {
        ...
        if (stock > 0 && user_balance >= $(this).data("points")) {
          ...
          let button_text =
            Number($(this).data("points")).toFixed(8) + " Points";
          $(this).html(button_text);
        } else if (stock == 0) {
          $(this).html("Sold Out");
          if (chain_id == "1") {
        ...
        $(
          "#farmStockInfo" + $(this).data("erc1155") + $(this).data("tokenId")
        ).html(stock);
        let farm_data = await window.tncLib.farmNftData(
          farm_address,
          $(this).data("erc1155"),
        ...
        $(
          "#farmTotalSupplyInfo" +
            $(this).data("erc1155") +
            $(this).data("tokenId")
        ).html(farm_data.supply);
        let nft = await window.tncLib.getNft(
          $(this).data("erc1155"),
          $(this).data("tokenId")
        );
        $(
          "#farmOwnInfo" + $(this).data("erc1155") + $(this).data("tokenId")
        ).html(nft.balance);
      });
    }
  ...
  this.observeChanges2 = async function () {
    ...
    if (farm_address != null && (await tncLib.isFarm(farm_address))) {
      let _null = web3.utils.toBN("0");
      let farmBalance = web3.utils.toBN(
        await tncLib.farmBalanceOfRaw(farm_address, tncLib.account)
      );
      let maxStakesLeft = web3.utils
        .toBN(await tncLib.farmMaxStakeRaw(farm_address))
        .sub(farmBalance);
      let decimals = await tncLib.farmTokenDecimals(farm_address);
      let farmToken = await tncLib.farmToken(farm_address);
      let balanceOfErc20 = web3.utils.toBN(
        await tncLib.balanceOfErc20Raw(farmToken, tncLib.account)
      );

      let pending = await tncLib.farmPendingWithdrawals(
        tncLib.account,
        farm_address
      );
      if (pending > 0) {
      ...
      $("#withdrawFeesBody").html(
        "Your current fee balance: " +
          _this.formatNumberString(pending, 18) +
          " " +
          getCurrency()
      );
      ...
      if (balanceOfErc20.gte(maxStakesLeft)) {
        $("#farmStakeMax").text(
          _this.formatNumberString(maxStakesLeft.toString(), decimals) +
            " " +
            (await tncLib.farmTokenSymbol(farm_address))
        );
      } else if (
        balanceOfErc20.lte(maxStakesLeft) &&
        balanceOfErc20.gt(_null)
      ) {
        $("#farmStakeMax").text(
          _this.formatNumberString(balanceOfErc20.toString(), decimals) +
            " " +
            (await tncLib.farmTokenSymbol(farm_address))
        );
      } else {
        $("#farmStakeMax").text(
          0 + " " + (await tncLib.farmTokenSymbol(farm_address))
        );
      }
      ...
      if (farmBalance.gt(_null)) {
        $("#farmUnstakeMax").text(
          _this.formatNumberString(farmBalance.toString(), decimals) +
            " " +
            (await tncLib.farmTokenSymbol(farm_address))
        );
      } else {
  ...
  this.updateNftRemoval = async function () {
    ...
    let nftBalance = await tncLib.balanceof(
    ...
    if (nftBalance < amount) {
      _alert(
        "Balance exceeds desired amount. Farm's NFT balance: " + nftBalance
      );
      return;
    }
  ...
  this.populateShopPrice = async function (e) {
    ...
    $("#farmShopEditControllerPrice").val(
      _this.formatNumberString(prices[0], 18)
    );
  ...
  this.updateEditNft = async function () {
    ...
    if (points == "" || parseFloat(points) <= 0) {
      _alert("Please enter a positive amount of redemption points.");
      return;
    }
    if (mintFee == "") {
      _alert("Please enter an artist fee.");
      return;
    }
    if (controllerFee == "") {
      _alert("Please enter a controller fee.");
      return;
    }
    if (
      parseFloat(mintFee) > 0 &&
      (!web3.utils.isAddress(artist) || artist == "")
    ) {
      _alert("Please enter a valid artist address if artist fee is set.");
      return;
    }
  ...
  this.updateShopPrice = async function () {
    ...
    if (mintFee == "") {
      _alert("Please enter an artist price.");
      return;
    }
    if (controllerFee == "") {
      _alert("Please enter a controller price.");
      return;
    }
  ...
  this.populateEditNft = async function (e) {
    ...
    let data = await tncLib.farmNftData(
      _this.getUrlParam("address"),
      farmAddress,
      id
    );
    ...
    $("#farmEditArtistFee").val(
      _this.formatNumberString("" + data.mintFee, 18)
    );
    $("#farmEditControllerFee").val(
      _this.formatNumberString("" + data.controllerFee, 18)
    );
  ...
  this.addNft = async function () {
    ...
    if (erc1155 == "" || id == "") {
      _alert("Please select an NFT from your wallet.");
      return;
    }
    if (amount == "" || parseInt(amount) <= 0) {
      _alert("Please enter a positive amount of NFTs to add.");
      return;
    }
    let myNfts = await tncLib.getMyNfts(erc1155);
    if (myNfts.length == 0) {
      _alert("You don't own enough of this NFT.");
      return;
    }
    if (points == "" || parseFloat(points) <= 0) {
      _alert("Please enter a positive amount of redemption points.");
    ...
    if (mintFee == "") {
      _alert("Please enter an artist fee in ETH.");
      return;
    }
    if (controllerFee == "") {
      _alert("Please enter an controller fee in ETH.");
      return;
    }
    if (
      parseFloat(mintFee) > 0 &&
    ...
    controllerFee = web3.utils.toBN(
      _this.resolveNumberString(controllerFee, 18)
    );

    let approved = await tncLib.erc1155IsApprovedForAll(
      tncLib.account,
      _this.getUrlParam("address"),
      erc1155
    );
    ...
    if (approved) {
      ...
      tncLib.erc1155SetApprovalForAll(
        ...
        function (err) {
          ...
          $("#addNftsButton").html("Add");
          let errMsg =
            "An error occurred with your set approval for all transaction.";
          toastr["error"](errMsg, "Error");
        }
      );
    }
  ...
  this.populateWallet = async function () {
    ...
    switch ($("#farmNftCollection").val()) {
      ...
      default:
        if (
          !web3.utils.isAddress($("#farmCustomErc1155Address").val().trim())
        ) {
        ...
        if (
          !(await tncLib.isErc1155Supported(
            $("#farmCustomErc1155Address").val().trim()
          ))
        ) {
          $("#walletModalBody").html(
            "Given address does not support the ERC1155 standard."
          );
          return;
        }
    ...
    for (let i = 0; i < addresses.length; i++) {
      data.push({
        erc1155: addresses[i],
        ids: await tncLib.getMyNftsByUri(addresses[i]),
      });
    }
  ...
  this.addWalletRow = async function (erc1155, id) {
    ...
    let code =
      '<div style="cursor: pointer;border-bottom: 1px solid #ccc" class="row mb-2 pb-2" id="walletEntry' +
      erc1155 +
      id +
      '">';
    ...
    switch (page) {
      default:
        ...
        if (farm_address != null && (await tncLib.isFarm(farm_address))) {
          ...
          $("#farmSetShopPriceModal").on(
            "show.bs.modal",
            _this.populateShopPrice
          );
          ...
          $("#farmRemoveNftsModal").on(
            "show.bs.modal",
            _this.populateNftRemoval
          );
          ...
          $("#farmTokensStaked").html(
            Number(
              await tncLib.farmBalanceOf(farm_address, tncLib.account)
            ).toFixed(8)
          );

          clearInterval(_this.tokenStakingInterval);
          _this.tokenStakingInterval = setInterval(async function () {
            $("#farmTokensStaked").html(
              Number(
                await tncLib.farmBalanceOf(farm_address, tncLib.account)
              ).toFixed(8)
          ...
          $("#farmPointsEarned").html(
            Number(
              await tncLib.farmPointsEarned(farm_address, tncLib.account)
            ).toFixed(9)
          );

          clearInterval(_this.earnedStakingInterval);
          _this.earnedStakingInterval = setInterval(async function () {
            $("#farmPointsEarned").html(
              Number(
                await tncLib.farmPointsEarned(farm_address, tncLib.account)
              ).toFixed(9)
