function TncLibMarket() {
  ...
  if (chain_id === "4") {
    ...
    this.market = new web3.eth.Contract(
      marketABI,
      "0xd799e8dA5b767328A8201d4ca759bE7AD2BeD3aA",
      { from: this.account }
    );
    this.swap = new web3.eth.Contract(
      swapABI,
      "0xDeBaEfa50E0CA793F062234D0D44F0f64dd9eB5d",
      { from: this.account }
    );
    this.wrap = new web3.eth.Contract(
      marketWrapABI,
      "0xC2c4E274F64501CDFE307877c0E42621A137d696",
      { from: this.account }
    );
    this.account = "";
    ...
    this.wrap = new web3.eth.Contract(marketWrapABI, "", {
      from: this.account,
    });
    this.account = "";
    ...
    this.market = new web3.eth.Contract(
      marketABI,
      "0xB62b9EA9453942dC884190b07D36C79727C59505",
      { from: this.account }
    );
    this.swap = new web3.eth.Contract(
      swapABI,
      "0x1E4fF0af2e269B659D3D18b984036d93B99B28d2",
      { from: this.account }
    );
    this.wrap = new web3.eth.Contract(
      marketWrapABI,
      "0xBD541908B498De6fD3a88055ceCE6a838de5053f",
      { from: this.account }
    );
    this.account = "";
    ...
    this.market = new web3.eth.Contract(
      marketABI,
      "0x2d5cf3610e496Db8c25895302Ff8201a9c7fd766",
      { from: this.account }
    );
    this.swap = new web3.eth.Contract(
      swapABI,
      "0x19bC9705c2f620C41974BF2f8f12180c608a8ee6",
      { from: this.account }
    );
    this.wrap = new web3.eth.Contract(
      marketWrapABI,
      "0x86b71b4742D8Dbc645B01062adbd4b07455934FB",
      { from: this.account }
    );
    this.account = "";
    ...
    this.market = new web3.eth.Contract(
      marketABI,
      "0x76D99F15894A3C7038d2C9c7215733d8107A2Da3",
      { from: this.account }
    );
    this.swap = new web3.eth.Contract(
      swapABI,
      "0x3Fbf41d865f86e5950CBB7f72926Dc2B0Cc5036b",
      { from: this.account }
    );
    this.wrap = new web3.eth.Contract(
      marketWrapABI,
      "0x0fe8e7051b31a63d9da26815a999990463B53f94",
      { from: this.account }
    );
    this.account = "";
    ...
    this.market = new web3.eth.Contract(
      marketABI,
      "0xF42421e2FFA453762C75010aac3b688598F87494",
      { from: this.account }
    );
    this.swap = new web3.eth.Contract(
      swapABI,
      "0xEB2F7E7162f8D6FefD3ae7E5e1Ab63BA192FFBa9",
      { from: this.account }
    );
    this.wrap = new web3.eth.Contract(
      marketWrapABI,
      "0x14a870A37116EB70Bab88739a99a529cfac04C9d",
      { from: this.account }
    );
    this.account = "";
  } else {
  ...
  this.saleExists = async function (account, index) {
    ...
    return await this.market.methods
      .saleExists(account, index)
      .call({ from: this.account });
  };

  this.getSwapRequestsLength = async function (account) {
    await sleep(sleep_time);
    return await this.swap.methods
      .getSwapRequestsLength(account)
      .call({ from: this.account });
  };
  ...
  this.getSwapRequestsListLength = async function (account) {
    await sleep(sleep_time);
    return await this.swap.methods
      .getSwapRequestsListLength(account)
      .call({ from: this.account });
  };
  ...
  this.getSwapRequestListEntry = async function (account, index) {
    await sleep(sleep_time);
    return await this.swap.methods
      .getSwapRequestListEntry(account, index)
      .call({ from: this.account });
  };
  ...
  this.getSwapExists = async function (seller0, seller1, index0) {
    ...
    return await this.swap.methods
      .swapStakers(
        seller0,
        seller1,
        "0x" + web3.utils.padLeft(_index0.toString(16), 64)
      )
      .call({ from: this.account });
  };
  ...
  this.getAsk = async function (index, account) {
    ...
    if (account != "") {
      askIndex = await this.market.methods
        .userAsks(account, index)
        .call({ from: this.account });
    } else {
      askIndex = await this.market.methods
        .publicAsks(index)
  ...
  this.getCategoriesLength = async function (category) {
    ...
    return await this.wrap.methods
      .getCategoriesLength(category)
      .call({ from: this.account });
  };
  ...
  this.getCategory = async function (category, index) {
    ...
    return await this.wrap.methods
      .categories("0x" + web3.utils.padLeft(_cat.toString(16), 64), index)
      .call({ from: this.account });
  };
  ...
  this.getRoyalties = async function (erc1155Address, id) {
    ...
    return await this.market.methods
      .royalties(
        erc1155Address,
        "0x" + web3.utils.padLeft(_id.toString(16), 64)
      )
      .call({ from: this.account });
  };
  ...
  this.sell = async function (
    ...
    try {
      gas = await this.wrap.methods
        .sell(erc1155Address, id, amount, token, setup)
        .estimateGas({
          from: this.account,
        });
    } catch (e) {
  ...
  this.setRoyalties = async function (
    erc1155Address,
    id,
    royaltyPercent,
    preCallback,
    ...
    try {
      gas = await this.market.methods
        .setRoyalty(erc1155Address, id, royaltyPercent)
        .estimateGas({
          from: this.account,
        });
    } catch (e) {
  ...
  this.buy = async function (
    ...
    this.market.methods
      ...
      .on("receipt", function (receipt) {
        console.log("Acquire order placed.");
        postCallback(receipt);
      });
  };
  ...
  this.withdrawFunds = async function (
    token,
    preCallback,
    postCallback,
    errCallback
  ...
  this.requestSwap = async function (
    index0,
    index1,
    nif,
    preCallback,
    ...
    try {
      gas = await this.swap.methods
        .requestSwap(index0, index1, nif)
        .estimateGas({
          from: this.account,
        });
    } catch (e) {
  ...
  this.acceptSwap = async function (
    swapIndex,
    preCallback,
    postCallback,
    errCallback
  ...
  this.cancelSwapRequest = async function (
    seller,
    swapIndex,
    preCallback,
    postCallback,
    ...
    try {
      gas = await this.swap.methods
        .cancelSwapRequest(seller, swapIndex)
        .estimateGas({
          from: this.account,
        });
    } catch (e) {
